...
class SVEA2(RAD):
    def __init__(self, obs_shape, action_shape, args):
        ...
        self.weak_critic = deepcopy(self.critic)
        self.weak_critic_optimizer = torch.optim.Adam(
            self.weak_critic.parameters(),
            lr=args.critic_lr,
    ...
    def update_critic(self, obs, action, reward, next_obs, not_done, L=None, step=None):
        ...
        current_Q1, current_Q2 = self.weak_critic(obs, action)
        weak_critic_loss_shift = F.mse_loss(current_Q1, target_Q) + F.mse_loss(
            current_Q2, target_Q
        )

        self.weak_critic_optimizer.zero_grad()
        weak_critic_loss_shift.backward()
        self.weak_critic_optimizer.step()

    def soft_update_critic_target(self):
        utils.soft_update_params(
            self.weak_critic.Q1, self.critic_target.Q1, self.critic_tau
        )
        utils.soft_update_params(
            self.weak_critic.Q2, self.critic_target.Q2, self.critic_tau
        )
        utils.soft_update_params(
            self.weak_critic.encoder, self.critic_target.encoder, self.encoder_tau
        )
